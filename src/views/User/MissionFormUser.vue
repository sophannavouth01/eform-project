		...
		<form @submit.prevent="submitForm">
			...
			<div
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					...
					<div>
						<label for="desination-location">Desination/ <span
								class="battambang-regular pt-2 text-[16px]">ទីតាំងដែលត្រូវចុះបេសកម្ម</span></label>
						<n-select v-model:value="form.desinationlocation" :options="siteOptions" label-field="label"
							value-field="value" class="mt-3 text-[16px]" />
...
const siteOptions = computed(() => {
	return sites.value.map(site => ({
		label: site.name,
		value: site.name,  // Use name for both label and value
	}));
});
...
const submitForm = async () => {
	...
	if (!form.value.FromDate || !form.value.FromTime || !form.value.ToDate || !form.value.ToTime || !form.value.BackDate || !form.value.BackTime || !form.value.ReasonForLeave || !form.value.NumberOfDayrequested || !form.value.desinationlocation || !form.value.Phone || !form.value.Gender || !form.value.LeaveType || !employeeId.value) {
	...
	const confirmMessage = `
	  <div style="text-align: left;">
		...
		<p class="p-custom"><span class="custom-width">ទីតាំងដែលត្រូវចុះបេសកម្ម:</span> <span class="custom-span">${form.value.desinationlocation}</span></p>
...
@media (max-width: 600px) {
	...
	/* Ensure the text banner is responsive */
	.bg .text-2xl {
		font-size: 1.5rem;
		/* Adjust font size for small screens */
		padding: 10px 0;
		/* Add some padding to separate from the edges */
		text-align: center;
